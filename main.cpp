...
int main(int argc, char** argv){
    ...
    if(annSearch == "OpenCV_KD"){
        cv::Mat covdetDMat = vec2cvMat_2D(covdetD.desctor);
        ...
        for(int i = 0; i < covdetQ.desctor.size(); i++){
            flann_index.knnSearch(covdetQ.desctor[i], tmpIdx, tmpDis, numNeighbors, cv::flann::SearchParams(128));
    ...
    if(annSearch == "VLfeat_KD"){
        float * descsQ = vectors2OneArray(covdetQ.desctor); // 将特征拼成一位数组
        float * descsD = vectors2OneArray(covdetD.desctor);
        int numTrees = 8;
        int maxNumComparisions = 800;
        VlKDForest * forest = kdtreebuild(descsD, covdetD.desctor.size(), numTrees, maxNumComparisions, descsQ, covdetQ.desctor.size(), numNeighbors, idx, dis);
        vl_kdforestquery(forest, numNeighbors, (int)covdetQ.desctor.size(), descsQ, idx, dis);
        delete descsQ;
        delete descsD;
    }
